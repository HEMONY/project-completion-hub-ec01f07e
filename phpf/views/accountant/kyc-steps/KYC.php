...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Save POST fields safely (NO overwrite)
    foreach ($_POST as $key => $value) {
        $_SESSION['form']['step1'][$key] = $value;
    }

    // Save ALL uploaded files (one submit = many files)
    ...
    if (!empty($_FILES['upload_trade_license']['name'][0])) {
        $_SESSION['form']['step1']['trade_license_files'] = [];
        
        foreach ($_FILES['upload_trade_license']['tmp_name'] as $i => $tmp) {
            if ($_FILES['upload_trade_license']['error'][$i] !== UPLOAD_ERR_OK) {
                continue;
            }
            
            $file_size = $_FILES['upload_trade_license']['size'][$i];
            ...
            $_SESSION['form']['step1']['trade_license_files'][] = [
                'filename' => $_FILES['upload_trade_license']['name'][$i],
                'mime'     => $_FILES['upload_trade_license']['type'][$i],
                'size'     => $file_size,
                'data'     => base64_encode($file_content)
            ];
        }
    ...
    // Save authorization letter files
    if (!empty($_FILES['upload_authorization_letter']['name'][0])) {
        $_SESSION['form']['step1']['authorization_letter_files'] = [];
        
        foreach ($_FILES['upload_authorization_letter']['tmp_name'] as $i => $tmp) {
            if ($_FILES['upload_authorization_letter']['error'][$i] !== UPLOAD_ERR_OK) {
                continue;
            }
            
            $file_size = $_FILES['upload_authorization_letter']['size'][$i];
            ...
            $_SESSION['form']['step1']['authorization_letter_files'][] = [
                'filename' => $_FILES['upload_authorization_letter']['name'][$i],
                'mime'     => $_FILES['upload_authorization_letter']['type'][$i],
                'size'     => $file_size,
                'data'     => base64_encode($file_content)
            ];
        }
...
<head>
  ...
  <style>
    ...
    .authorization-info p:last-child {
        margin-bottom: 0;
    }
...
<body>
    ...
    <form method="POST" action="" enctype="multipart/form-data" id="kyc-form">
    <!-- المحتوى الرئيسي -->
            ...
            <div id="kyc-fields" style="display:none;">
                ...
                <div class="attachments-container">
                    ...
                    <div class="form-group" id="trade-license-upload-group" style="display: none;">
                        ...
                        <input type="file" id="upload-trade-license" accept=".pdf,.jpg,.jpeg,.png" name="upload_trade_license[]" />
                    ...
                    <div class="authorization-letter-section">
                        <div class="form-group">
                            <label for="upload-authorization-letter">
                                Upload Authorization Letter:
                                <span class="tooltip-icon">
                                    +
                                    <div class="tooltip-content">
                                        Upload the completed and signed authorization letter. The template can be downloaded below.
                                    </div>
                                </span>
                            </label>
                            <div class="authorization-info">
                                <p>Please download the template, complete all required fields, obtain your client's signature, and upload the duly completed document.</p>
                            </div>
                            <a class="down" onclick="downloadAuthorizationTemplate()">
                                Download Authorization Letter Template
                            </a>
                            <br>
                            <input type="file" id="upload-authorization-letter" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" name="upload_authorization_letter[]" style="margin-top: 15px;" />
                            <div class="uploaded-files" id="authorization-letter-files">
                                <!-- الملفات المرفوعة ستظهر هنا -->
                            </div>
                        </div>
                    </div>
...
const authorizationLetterUpload = document.getElementById('upload-authorization-letter');
...
const authorizationLetterFilesContainer = document.getElementById('authorization-letter-files');
...
let uploadedAuthorizationLetterFiles = [];
...
// *************** وظيفة تحميل نموذج خطاب التفويض ***************
function downloadAuthorizationTemplate() {
    const a = document.createElement('a');
    a.href = './auth.docx';   // same directory
    a.download = 'auth.docx';
...
function setupFormSubmission() {
    kycForm.addEventListener('submit', async function(e) {
        ...
        // ملفات خطاب التفويض
        if (authorizationLetterUpload && authorizationLetterUpload.files.length > 0) {
            for (let i = 0; i < authorizationLetterUpload.files.length; i++) {
                formData.append('upload_authorization_letter[]', authorizationLetterUpload.files[i]);
                console.log(`  ملف خطاب التفويض ${i + 1}: ${authorizationLetterUpload.files[i].name}`);
            }
        }
...
function updatePlaceholders() {
    ...
    updateFileDisplay(uploadedAuthorizationLetterFiles, authorizationLetterFilesContainer, true);
}
...
function setupFileUploadListeners() {
    ...
    // مستمع لتحميل خطاب التفويض
    if (authorizationLetterUpload) {
        authorizationLetterUpload.addEventListener('change', function(e) {
            handleFileUpload(e.target.files, uploadedAuthorizationLetterFiles, authorizationLetterFilesContainer, true);
        });
    }
}
    ...
    fileArray.forEach((fileData, index) => {
        ...
        fileItem.innerHTML = `
            ...
            <div class="file-actions">
                <button type="button" class="download-btn" onclick="downloadFileFromBase64(${index}, '${container.id}')">Download</button>
                <button type="button" class="delete-btn" onclick="deleteFile(${index}, '${container.id}')">Delete</button>
            </div>
        `;
...
function downloadFileFromBase64(index, containerId) {
    let fileArray;
    
    // تحديد مصفوفة الملفات بناءً على الحاوية
    if (containerId === 'trade-license-files') {
        fileArray = uploadedTradeLicenseFiles;
    } else if (containerId === 'authorization-letter-files') {
        fileArray = uploadedAuthorizationLetterFiles;
    } else {
        fileArray = uploadedIdPassportFiles;
    }
...
function deleteFile(index, containerId) {
    let fileArray;
    let container;
    let singleFile = false;
    
    // تحديد مصفوفة الملفات والحاوية بناءً على containerId
    if (containerId === 'trade-license-files') {
        fileArray = uploadedTradeLicenseFiles;
        container = tradeLicenseFilesContainer;
        singleFile = true;
    } else if (containerId === 'authorization-letter-files') {
        fileArray = uploadedAuthorizationLetterFiles;
        container = authorizationLetterFilesContainer;
        singleFile = true;
    } else {
        fileArray = uploadedIdPassportFiles;
        container = idPassportFilesContainer;
        singleFile = false;
    }
...
function createShareholderRow() {
    ...
    row.innerHTML = `
        <td><input type="text" placeholder="Full Name" name="shareholder_name[]" required oninput="validateAll(); updateManagementSelectOnChange()"></td>
...
function createUBORow() {
    ...
    row.innerHTML = `
        <td><input type="text" placeholder="Full Name" name="ubo_name[]" required oninput="validateAll(); updateManagementSelectOnChange()"></td>
...
function createManagementRow() {
    ...
    row.innerHTML = `
        <td><input type="text" placeholder="Full Name" name="management_name" required></td>
...
function validateAll() {
    ...
    // التحقق من خطاب التفويض
    if (uploadedAuthorizationLetterFiles.length === 0) {
        errors.push("Please upload the Authorization Letter. Download the template, fill it, sign it, and upload the completed document.");
    }
